...
class CaixaImoveisScraperCorrigido:
    def __init__(self):
    ...
    def buscar_imoveis_joinville(self):
        """Busca imóveis em Joinville usando requests com os nomes corretos dos campos"""
        try:
            ...
            if response.status_code == 200:
                ...
                # Agora tenta fazer a busca com os nomes corretos dos campos
                params = {
                    'cmb_estado': 'SC',  # Nome correto do campo
                    'cmb_cidade': 'JOINVILLE',  # Nome correto do campo
                    'cmb_modalidade': 'Selecione',  # Modalidade (pode deixar padrão)
                    'cmb_tp_imovel': 'Indiferente',  # Tipo de imóvel
                    'cmb_quartos': 'Indiferente',  # Quartos
                    'cmb_vg_garagem': 'Indiferente',  # Vagas de garagem
                    'cmb_area_util': 'Indiferente',  # Área útil
                    'cmb_faixa_vlr': 'Indiferente',  # Faixa de valor
                    'btnBuscar': 'Buscar'
                }
                
                print("Fazendo busca com parâmetros corretos...")
    ...
    def salvar_resultados(self, formato='csv'):
        ...
        if formato == 'csv':
            filename = f"imoveis_joinville_corrigido_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            df.to_csv(filename, index=False, encoding='utf-8-sig')
            print(f"Resultados salvos em: {filename}")
        
        elif formato == 'json':
            filename = f"imoveis_joinville_corrigido_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def executar_busca(self):
        ...
        try:
            print("=== SCRAPER CORRIGIDO - IMÓVEIS CAIXA JOINVILLE ===")
...
def main():
    scraper = CaixaImoveisScraperCorrigido()
